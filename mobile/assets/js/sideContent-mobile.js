...
sideContentContainer?.addEventListener("click", function (event) {
    sideContentContainer.classList.add("sideContentContainerActive");
    // event.stopPropagation(); // جلوگیری از انتشار به `document`
});

sideContent?.addEventListener("click", function (event) {
    sideContent.classList.add("sideContentActive");
    sideContent.parentElement.classList.add("sideContentContainerActive");

    // event.stopPropagation(); // جلوگیری از انتشار به `document`

});

// حذف کلاس‌ها هنگام کلیک در خارج از `sideContentContainer`
document.addEventListener("click", function (event) {
    if (!sideContentContainer?.contains(event.target)) {
        sideContentContainer?.classList.remove("sideContentContainerActive");
        sideContent?.classList.remove("sideContentActive");
    }
});

let arrBtm = document.querySelector(".arrBtm");


let isFirstClick = true; // فلگ برای کنترل اولین کلیک

sideContent?.addEventListener("click", function (event) {
    event.stopPropagation(); // جلوگیری از انتشار کلیک

    if (isFirstClick) {
        sideContent.classList.add("sideContentActive");
        isFirstClick = false; // بعد از اولین کلیک، فلگ رو غیرفعال می‌کنیم
    }
});

arrBtm?.addEventListener("click", function (event) {
    event.stopPropagation(); // جلوگیری از انتشار کلیک
    sideContent?.classList.remove("sideContentActive");
});

// حذف کلاس‌ها هنگام کلیک بیرون از `sideContent`
document.addEventListener("click", function (event) {
    if (!sideContentContainer?.contains(event.target) && !sideContent?.contains(event.target)) {
        sideContentContainer?.classList.remove("sideContentContainerActive");
        sideContent?.classList.remove("sideContentActive");
    }
});
